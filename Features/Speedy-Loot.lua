...
--[[
    ...
    Gated by ns.db.profile.speedyLoot, and always defers to Master-Looter.lua
    when ns:WillAutoMasterLoot() is true — LootSlot calls in an ML session
    race the distribution path and pop the candidate dropdown for items the
    engine would have placed automatically.
]]
local _, ns = ...
...
--[[
    ...
    legacy global on older Classic builds. Returns freeSlots, bagFamily.
...
local function CountFreeBagSlots()
    ...
    for bagIndex = 0, NUM_BAG_SLOTS do
        local freeInBag, bagFamily = GetContainerNumFreeSlotsCompat(bagIndex)
...
local function HandleLootReady()
    if not ns.db or not ns.db.profile.speedyLoot then
        return
    end
