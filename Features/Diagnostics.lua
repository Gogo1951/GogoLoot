...
--[[
    ...
    probed here: both the trade watcher and the master-loot error correlation
    match on numeric message ids resolved through GetGameMessageInfo, and read
    those globals only as a fallback for a client where the constants resolved
    to no id at all. Which constants did resolve is reported by the Loot Method
    section, since an existence check cannot show that.
]]
ns.DIAGNOSTIC_API_CHECKS = {
	...
	{
		"Enum.LootMethod.MasterLoot",
		function()
			return type(Enum) == "table" and type(Enum.LootMethod) == "table" and Enum.LootMethod.MasterLoot ~= nil
		end,
	},
	...
	--[[
        ...
        resolved is reported by the Loot Method section below, since an
	...
	--[[
        ...
        themselves are in the Loot Method section.
    ]]
	{
		"ERR_TRADE_COMPLETE",
		function()
