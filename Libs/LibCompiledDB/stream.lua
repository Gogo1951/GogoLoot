...
function LibCompiledDB.stream:GetStream(mode) -- returns a new stream
    ...
    for k,v in pairs(LibCompiledDB.stream) do -- copy functions to new object
        stream[k] = v
    end
    stream._pointer = 1
    stream._bin = {}
    ...
    if mode then
        stream._mode = mode
        if mode == "raw" then
            stream.ReadByte = LibCompiledDB.stream._ReadByte_raw
            stream.ReadTinyString = LibCompiledDB.stream._ReadTinyStringBySubstring
            stream.ReadShortString = LibCompiledDB.stream._ReadShortStringBySubstring
            stream.ReadTinyStringNil = LibCompiledDB.stream._ReadTinyStringNilBySubstring
            stream._WriteByte = LibCompiledDB.stream._writeByte
        elseif mode == "1short" then
            stream.ReadByte = LibCompiledDB.stream._ReadByte_1short
            stream._WriteByte = LibCompiledDB.stream._WriteByte_1short
        else
            stream.ReadByte = LibCompiledDB.stream._ReadByte_b89
            stream._WriteByte = LibCompiledDB.stream._WriteByte_b89
...
function LibCompiledDB.stream:_writeByte(val)
...
function LibCompiledDB.stream:_readByte()
    self._pointer = self._pointer + 1
    return self._bin[self._pointer-1]
end

function LibCompiledDB.stream:WriteByte(val)
    self:_WriteByte(val)
    self._raw_bin[self._rawptr] = val
    self._rawptr = self._rawptr + 1
end

function LibCompiledDB.stream:SetPointer(pointer)
    self._pointer = pointer;
end

function LibCompiledDB.stream:_ReadByte_b89()
...
function LibCompiledDB.stream:_WriteByte_b89(e)
    if e == nil then
        return 
    end
...
function LibCompiledDB.stream:_WriteByte_1short(e)
...
function LibCompiledDB.stream:_ReadByte_1short()
    local v = self:_readByte()
...
function LibCompiledDB.stream:_WriteByte_raw(e)
    self:_writeByte(e)
end

function LibCompiledDB.stream:_ReadByte_raw()
...
--function LibCompiledDB.stream:ReadByte()
...
function LibCompiledDB.stream:ReadBytes(count)
    local ret = {};
    for i=1, count do
...
function LibCompiledDB.stream:ReadShorts(count)
    local ret = {};
    for i=1, count do
...
function LibCompiledDB.stream:ReadTinyString()
    local length = self:ReadByte()
    local ret = {};
    for i = 1, length do
...
function LibCompiledDB.stream:_ReadTinyStringBySubstring()
    local length = self:ReadByte()
...
function LibCompiledDB.stream:_ReadTinyStringNilBySubstring()
    local length = self:ReadByte()
...
function LibCompiledDB.stream:ReadTinyStringNil()
    local length = self:ReadByte()
    if length == 0 then return nil end
    local ret = {};
    for i = 1, length do
...
function LibCompiledDB.stream:ReadShortString()
...
function LibCompiledDB.stream:_ReadShortStringBySubstring()
    local length = self:ReadShort()
    local ret = string.sub(self._bin, self._pointer, self._pointer+length-1)
    self._pointer = self._pointer + length
    return ret
...
function LibCompiledDB.stream:WriteBytes(...)
    for _,val in pairs({...}) do
        self:WriteByte(val)
    end
end

function LibCompiledDB.stream:WriteShorts(...)
    for _,val in pairs({...}) do
        self:WriteShort(val)
    end
end

function LibCompiledDB.stream:WriteShort(val)
    --print("wshort: " .. val);
    self:WriteByte(mod(rshift(val, 8), 256));
    self:WriteByte(mod(val, 256));
end

function LibCompiledDB.stream:WriteInt(val)
...
function LibCompiledDB.stream:WriteInt24(val)
    --print("wi24: " .. val);
    self:WriteByte(mod(rshift(val, 16), 256));
    self:WriteByte(mod(rshift(val, 8), 256));
    self:WriteByte(mod(val, 256));
...
function LibCompiledDB.stream:WriteInt12Pair(val1, val2)
...
function LibCompiledDB.stream:WriteLong(val)
...
function LibCompiledDB.stream:WriteTinyString(val)
    self:WriteByte(string.len(val));
    for i=1, string.len(val) do
...
function LibCompiledDB.stream:WriteShortString(val)
...
function LibCompiledDB.stream:Save()
...
function LibCompiledDB.stream:SaveRaw()
...
function LibCompiledDB.stream:Load(bin)
    self._pointer = 1
...
function LibCompiledDB.stream:LoadRaw(bin)
    self._pointer = 1
    self._bin = bin
    self._level = 0
end
