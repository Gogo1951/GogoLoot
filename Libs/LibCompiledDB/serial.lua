LibCompiledDB.serial = {}

function LibCompiledDB.serial:Hash(value)
...
LibCompiledDB.serial.SerializerHashDB = {
}
LibCompiledDB.serial.SerializerHashDBReversed = {
}

local function addHash(str)
    local hash = LibCompiledDB.serial:Hash(str)
    if LibCompiledDB.serial.SerializerHashDBReversed[hash] then
        -- dont add, also prevents collissions 
        return
    end
    LibCompiledDB.serial.SerializerHashDB[str] = hash
    LibCompiledDB.serial.SerializerHashDBReversed[hash] = str
end
...
local function _ReadObject(self)
    ...
    return LibCompiledDB.serial.ReaderTable[typ](self);
end
...
LibCompiledDB.serial.ReaderTable = {
    ...
    [9] = function(self) return LibCompiledDB.serial.SerializerHashDBReversed[self.stream:ReadInt()] end,
...
LibCompiledDB.serial.WriterTable = {
    ["number"] = function(self, value)
        local _, fract = math.modf(value)
        if fract > 0 then
            LibCompiledDB.serial.WriterTable["float"](self, value)
        else
            local sign = 0
            if value < 0 then
    ...
    ["string"] = function(self, value)
        if LibCompiledDB.serial.SerializerHashDB[value] and string.len(value) > 4 then
            self.stream:WriteByte(9)
            self.stream:WriteInt(LibCompiledDB.serial.SerializerHashDB[value])
    ...
    ["table"] = function(self, value, depth)
        ...
        if isArray(value) then
            ...
            for _, v in pairs(value) do
                local t = type(v)
                if not LibCompiledDB.serial.WriterTable[t] then
                    print("LibCompiledDB.serial Error: Unhandled type: " .. t)
                else
                    LibCompiledDB.serial.WriterTable[t](self, v, depth)
                    if t == "string" then
                        addHash(v)
                    end
                end
            ...
            for key, v in pairs(value) do
                if key and v then
                    LibCompiledDB.serial:WriteKeyValuePair(key, v, depth + 1)
                end
            end
        end
    end,
...
LibCompiledDB.serial.enableObjectLimit = true

function LibCompiledDB.serial:WriteKeyValuePair(key, value, depth)
    if not value or not key then return; end
    if not depth then
        depth = 0
    end
    if self.objectCount > 8192 and false then print("[LibCompiledDB.serial] Too many objects in input table!") return end
    self.objectCount = self.objectCount + 1
    local keyType = type(key)
    local valueType = type(value)
    local writeKey = LibCompiledDB.serial.WriterTable[keyType]
    local writeValue = LibCompiledDB.serial.WriterTable[valueType]
    if not writeKey or not writeValue then
        print("LibCompiledDB.serial Error: Unhandled type: " .. keyType .. "  " .. valueType)
    else
...
function LibCompiledDB.serial:SetupStream(encoding)
    ...
    if self.stream and self.streamEncoding == encoding then
        ...
        self.stream = LibCompiledDB.stream:GetStream(encoding)
        self.streamEncoding = encoding
    end
    clearHashes()
end

function LibCompiledDB.serial:Serialize(tab, encoding)
    LibCompiledDB.serial:SetupStream(encoding)
    self.objectCount = 0
    --LibCompiledDB.serial:WriteKeyValuePair("meta", {protocolVersion = 1, mode="1short"})
    LibCompiledDB.serial:WriteKeyValuePair(1, tab)
    return self.stream:Save()
end

function LibCompiledDB.serial:Deserialize(data, encoding)
    LibCompiledDB.serial:SetupStream(encoding)
