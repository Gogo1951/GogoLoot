...
--[[
    AceDB-3.0 defaults table, passed to AceDB:New in Features/Core.lua.
    Every setting lives in the active profile (ns.db.profile); AceDB applies
    these defaults via metatables, so there is no manual merge anywhere.
    The empty ignoredItemsMaster / ignoredItemsSolo lists are seeded from
    the default lists below by Core's refill-on-empty rule, never here.
]]
...
ns.DATABASE_DEFAULTS = {
    ...
    global = { -- profile-independent, account-wide (minimap position only)
        minimap = {}
    }
}
